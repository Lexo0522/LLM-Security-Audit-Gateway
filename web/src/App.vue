...
async function removeUpstream(item: Upstream) {
  if (!window.confirm(`Delete upstream "${item.name}"? Gateway keys bound to it keep failing, and the upstream cannot be deleted while such keys exist.`)) return
  const result = await run(() => api.deleteUpstream(item.id), 'Upstream deleted.')
